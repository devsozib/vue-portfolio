...
tabMnu = tabWrapper.find('.tab-menu li'),
tabContent = tabWrapper.find('.tab-cont > .tab-item');
tabContent.not(':nth-child(1)').fadeOut(50);
tabMnu.each(function (i) {
$(this).attr('data-tab', 'tab' + i);
});
tabContent.each(function (i) {
...
tabMnu.on('click', function () {
var tabData = $(this).data('tab');
tabWrapper.find(tabContent).fadeOut(50);
tabWrapper.find(tabContent).filter('[data-tab=' + tabData + ']').fadeIn(50);
});
$('.tab-menu > li').on('click', function () {
var before = $('.tab-menu li.active');
before.removeClass('active');
...
function galleryMasonaryTwo(){
  ...
  $('.v-open-btn-two').on('click', function(){
    $('.version-change-area-two').toggleClass('active');
  });
